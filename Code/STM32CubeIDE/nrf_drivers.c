...
Written by:				Kyle Chong
Date Written:			6/18/2025
Last modified:		-/-
Description:			edit this later - This is an STM32 device driver library for the NRF24L01 Nordic Radio transceiver, using STM HAL libraries

References:				credit yaqoob - This library was written based on the Arduino NRF24 Open-Source library by J. Coliz and the NRF24 datasheet
...
static HAL_StatusTypeDef nrf24_xfer(uint8_t *tx, uint8_t *rx, uint16_t len)
{
    HAL_StatusTypeDef s =
        HAL_SPI_TransmitReceive(&nrf24_hspi, tx, rx, len, 100);
    if (s != HAL_OK)                // any timeout/overrun/etc.
        NRF24_csn(1);               // make sure CSN rises
    return s;
}
...
uint8_t NRF24_read_register(uint8_t reg)
{
	...
	spiBuf[0] = reg&0x1F;
	HAL_SPI_Transmit(&nrf24_hspi, &spiBuf[0], 1, 100);
	//Receive data
...
{
	NRF24_csn(0);
	uint8_t cmd = CMD_FLUSH_TX;       // 0xE1
	HAL_SPI_Transmit(&nrf24_hspi, &cmd, 1, 100);
	NRF24_csn(1);
}
...
bool NRF24_read(void* buf, uint8_t len)
{
    ...
    uint8_t n = MIN(len, NRF24_getPayloadSize());

    NRF24_csn(0);
    HAL_SPI_Transmit(&nrf24_hspi, &cmd, 1, 100);
    HAL_SPI_Receive(&nrf24_hspi, buf, n, 100);
    NRF24_csn(1);

    // only clear RX_DR flag here; DO NOT flush the FIFO
    NRF24_write_register(REG_STATUS, _BV(BIT_RX_DR));

    // tell caller whether more payloads remain
    uint8_t fs = NRF24_read_register(REG_FIFO_STATUS);
    return !(fs & _BV(BIT_RX_EMPTY));
}
...
{
    uint8_t cmd = CMD_R_RX_PL_WID;   // 0x60
    uint8_t wid = 0;

    NRF24_csn(0);
    HAL_SPI_Transmit(&nrf24_hspi, &cmd, 1, 100);
    HAL_SPI_Receive(&nrf24_hspi, &wid, 1, 100);
    NRF24_csn(1);

    if (wid > 32) {                  // invalid => flush
        NRF24_flush_rx();
        return 0;
    }
    return wid;
...
//27. Skipped, nrf24l01 (not plus) is out of date
...
void NRF24_startWrite(const void* buf, uint8_t len)
{
  ...
  // tpd2stby ≥ 1.5 ms
  HAL_Delay(2);
  uint8_t cfg = NRF24_read_register(0x00);
  printf("CONFIG after PWR_UP: 0x%02X\r\n", cfg);  // expect PWR_UP bit set
  // Load payload while CE is LOW
  NRF24_write_payload(buf, len);
  uint8_t fifo = NRF24_read_register(0x17);       // FIFO_STATUS
  printf("FIFO after load: TX_EMPTY=%u\r\n", (fifo>>4)&1);  // expect 0
  // CE HIGH ≥10 µs to kick TX
  NRF24_ce(1);
  NRF24_DelayMicroSeconds(2000);
  NRF24_ce(0);
}
...
void NRF24_writeAckPayload(uint8_t pipe, const void* buf, uint8_t len)
{
    ...
    uint8_t cmd = CMD_W_ACK_PAYLOAD | (pipe & 0x07);  // 0xA8..0xAD

    NRF24_csn(0);
    HAL_SPI_Transmit(&nrf24_hspi, &cmd, 1, 100);
    HAL_SPI_Transmit(&nrf24_hspi, (uint8_t*)buf, n, 100);
    NRF24_csn(1);
}
...
{
    uint8_t s = NRF24_read_register(0x07);
    char b[160];
    uint8_t rx_p_no = (s >> 1) & 0x07;
    ...
    sprintf(b,
      ...
      "-------------------------\r\n",
      s,
      (s>>6)&1, (s>>5)&1, (s>>4)&1, rx_p_no, s&1);
    HAL_UART_Transmit(&nrf24_huart,(uint8_t*)b, strlen(b), 10);
}
